...
const createId = () => (typeof crypto !== "undefined" && typeof crypto.randomUUID === "function" ? crypto.randomUUID() : `item_${Date.now()}`);
...
const getItemKind = (category: BasketCategory) => (category === "random_blind" ? "blind_draw" : "goods");
const getGoodsSubtype = (purpose: BasketPurpose, category: BasketCategory) =>
  purpose === "itabag" && category === "badge" ? "itaBag_badge" : "general";

const overrideLabel: Record<"BUY" | "THINK" | "SKIP", string> = {
  BUY: "買う",
  THINK: "保留",
  SKIP: "やめる",
};
...
export default function BasketPage() {
  ...
  const [items, setItems] = useState<DraftItem[]>([createDraftItem(), createDraftItem(), createDraftItem()]);
  ...
  useEffect(() => {
    ...
    if (!raw) {
      // eslint-disable-next-line react-hooks/set-state-in-effect
      setBasketId(createId());
      setHasLoadedStorage(true);
      return;
    }
    try {
      const parsed = JSON.parse(raw) as Partial<BasketInput & { items: DraftItem[]; totalBudgetInput?: string; basketId?: string }>;
      setBasketId(parsed.basketId ?? createId());
  ...
  useEffect(() => {
    if (typeof window === "undefined" || !hasLoadedStorage || !basketId) return;
  ...
  }, [basketId, shopContext, purpose, totalBudgetInput, hardCap, timePressure, decisiveness, styleMode, items, hasLoadedStorage]);
  ...
  }, [basketId, normalizedItems, totalBudgetJPY, shopContext, purpose, hardCap, timePressure, decisiveness, styleMode]);
  ...
  return (
    <div className={`${containerClass} flex min-h-screen flex-col gap-6 py-8`}>
      ...
      <Card className="space-y-4 border border-slate-200 bg-white text-slate-900 dark:border-white/10 dark:bg-white/6 dark:text-zinc-50">
        ...
        <label className="space-y-1 block"><span className="text-sm">shopContext</span><select className={inputBaseClass} value={shopContext} onChange={(e) => setShopContext(e.target.value as BasketShopContext)}>{shopContextOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>
        <label className="space-y-1 block"><span className="text-sm">purpose</span><select className={inputBaseClass} value={purpose} onChange={(e) => setPurpose(e.target.value as BasketPurpose)}>{purposeOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>
        <div className="space-y-2">
          <p className="text-sm">totalBudget</p>
          <div className="flex flex-wrap gap-2">
            {budgetPresets.map((preset) => (
              <Button key={preset} variant={totalBudgetInput === String(preset) ? "primary" : "outline"} className="px-4 py-2" onClick={() => setTotalBudgetInput(String(preset))}>¥{preset.toLocaleString("ja-JP")}</Button>
            ))}
          </div>
        ...
        <label className="space-y-1 block"><span className="text-sm">hardCap</span><select className={inputBaseClass} value={hardCap} onChange={(e) => setHardCap(e.target.value as BasketHardCap)}>{hardCapOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>
        <label className="space-y-1 block"><span className="text-sm">timePressure</span><select className={inputBaseClass} value={timePressure} onChange={(e) => setTimePressure(e.target.value as BasketTimePressure)}>{timePressureOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>
        <label className="space-y-1 block"><span className="text-sm">decisiveness</span><select className={inputBaseClass} value={decisiveness} onChange={(e) => setDecisiveness(e.target.value as BasketDecisiveness)}>{decisivenessOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>
        <label className="space-y-1 block"><span className="text-sm">styleMode</span><select className={inputBaseClass} value={styleMode} onChange={(e) => setStyleMode(e.target.value as StyleMode)}>{styleModeOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>
      </Card>
        ...
        <div className="space-y-4">
          {items.map((item, index) => (
            <div key={item.id} className="rounded-xl border border-border p-3 space-y-3 dark:border-white/10">
              <div className="flex items-center justify-between"><p className="text-sm font-semibold">#{index + 1}</p><Button variant="ghost" className="px-3 py-2" disabled={items.length <= 3} onClick={() => removeItem(item.id)}>削除</Button></div>
              ...
              <select className={inputBaseClass} value={item.category} onChange={(e) => updateItem(item.id, { category: e.target.value as BasketCategory })}>{categoryOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>
              <select className={inputBaseClass} value={item.rarity} onChange={(e) => updateItem(item.id, { rarity: e.target.value as BasketRarity })}>{rarityOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select>
              <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={item.mustBuy} onChange={(e) => updateItem(item.id, { mustBuy: e.target.checked })} />mustBuy（特典/頼まれ/期限）</label>
            </div>
          ))}
        </div>
      </Card>
        ...
        {!result ? (
          ...
          <div className="space-y-4">
            <div className="rounded-xl border border-border p-3 text-sm dark:border-white/10">
              ...
              <p className={result.totals.overJPY > 0 ? "text-rose-500" : "text-emerald-600"}>{result.totals.overJPY > 0 ? `予算オーバー: ¥${result.totals.overJPY.toLocaleString("ja-JP")}` : `残額: ¥${result.totals.remainingJPY.toLocaleString("ja-JP")}`}</p>
              {result.totals.mustBuyOnlyOverBudget ? <p className="text-rose-500">mustBuy項目だけで上限を超えています。</p> : null}
            </div>

            {([ ["BUY_NOW", result.buyNow], ["HOLD", result.hold], ["SKIP", result.skip], ] as const).map(([label, list]) => (
              <div key={label} className="space-y-2">
                ...
                {list.map((item) => (
                  <div id={`item-${item.id}`} key={`${label}_${item.id}`} className="rounded-xl border border-border p-3 text-sm dark:border-white/10">
                    <p className="font-semibold">{item.name} / ¥{item.priceJPY.toLocaleString("ja-JP")} / score {item.priorityScore}</p>
                    {item.singleResultOverride ? <p className="mt-1 inline-flex rounded-full bg-slate-200 px-2 py-0.5 text-xs dark:bg-white/20">単品診断済み: {overrideLabel[item.singleResultOverride.decision]}</p> : null}
                    <ul className="list-disc pl-5">{item.reasons.map((reason, index) => <li key={`${item.id}_${index}`}>{reason}</li>)}</ul>
                    <div className="mt-3">
                      <Button
                        variant="outline"
                        className="w-full"
                        onClick={() => {
                          const params = new URLSearchParams({
                            ...
                            styleMode,
                            itemName: item.name,
                            priceYen: String(item.priceJPY),
                            itemKind: getItemKind(item.category),
                            goodsSubtype: getGoodsSubtype(purpose, item.category),
                            basketId,
                            basketItemId: item.id,
                          });
                          router.push(`/confirm/settings?${params.toString()}`);
                        }}
                      >
                        単品で深掘り（短）
                      </Button>
            ...
            {result.cutList.length > 0 ? (
              <div className="space-y-2 rounded-xl border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900 dark:border-amber-400/40 dark:bg-amber-500/10 dark:text-amber-100">
                ...
                <ul className="list-disc pl-5">{result.cutList.map((item) => (<li key={`cut_${item.id}`}>{item.name}（¥{item.priceJPY.toLocaleString("ja-JP")}, score {item.priorityScore}）</li>))}</ul>
              </div>
            ) : null}
          </div>
        )}
