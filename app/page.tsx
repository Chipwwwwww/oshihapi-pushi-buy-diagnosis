  ...
  return (
    <div className={`${containerClass} safe-bottom flex min-h-screen flex-col gap-6 bg-transparent py-8 dark:bg-[#0b0f1a]`}>
      <header className="flex flex-col gap-2">
        ...
        <p className="text-xs text-slate-500 dark:text-zinc-400">CD・Blu-ray・書籍のメディア判断では VT 文脈も扱いますが、代表例は CD / BD 中心です。チケット・遠征・会員/サブスク・スパチャは対象外です。</p>
      ...
      <Card className="space-y-4 border border-accent/25 bg-white text-slate-900 shadow-sm shadow-accent/5 dark:border-accent/30 dark:bg-white/6 dark:text-zinc-50">
        <div className="space-y-3">
          ...
          <p className="text-xs text-slate-500 dark:text-zinc-400">例: VT / ユニット名 + 初回限定 Blu-ray、VT + 特典付きCD。公式グッズ・ボイス・遠征系はここでは広げません。</p>
        </div>
        <div className="grid gap-3">
          <textarea
            value={searchClue}
            onChange={(event) => setSearchClue(event.target.value)}
            placeholder={"例：作品名＋缶バッジ / VT＋初回限定 Blu-ray / VT＋特典付きCD"}
          ...
          />
          <p className="text-sm text-slate-600 dark:text-zinc-300">商品名がわからなくても、作品名・キャラ名・価格・特典の手がかりで使えます。VT 系はまず CD / BD を中心に、既存のメディア診断の範囲で扱います。</p>
        </div>
      </Card>
