...
## 1) Mode 用語（ここが混乱ポイント）
- FlowMode（診断深さ）: 急いで/じっくり/AI相談 → 問題数/時間
- StyleMode（文案世界観）: standard/kawaii/oshi → 文案/表現だけ変える
...
3) `.\post_merge_routine.ps1`
   - build ✅ が唯一裁判
   - parity not confirmed の場合：
     - host / branch / push / Vercel の deploy 状況を確認
...
- もし存在するなら消す/移動を推奨：`$env:USERPROFILE\package-lock.json`
