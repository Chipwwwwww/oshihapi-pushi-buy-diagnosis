...
export const SCENARIO_COVERAGE_MAP: Record<ScenarioKey, ScenarioCoverageEntry> = {
  ...
  media_purchase_decision: {
    ...
    homepagePriority: "primary",
    heroOrder: 4,
    shopperIntent: "CD・映像・書籍を今買うか、比較して決めたい（VT は CD / BD 中心）",
    recommendedEntryLabel: "CD・映像・書籍を買うべきか迷う",
    compactEntryHint: "仕様差・特典差・再生環境を比較できます。VT 文脈は CD / BD を中心にこの枠で扱います。",
    providerEcologyHint: "メディア系は再生/閲覧環境、特典差、限定版、再入手性の比較が中心です。VT 文脈も既存メディア provider の範囲で、まず CD / BD を中心に扱います。",
    ...
    verificationHint: "仕様違い、店舗別特典、視聴環境、在庫状況は外部で最終確認してください。VT は特に CD / BD で、版違い・法人特典・初回条件の確認が重要です。",
    shortVerificationHint: "仕様差・法人/店舗特典・在庫は外部で最終確認。",
    scopeDisclosure: "メディア購入判断は強めで、VT も既存 media 枠の延長として主に CD / BD を扱います。書籍系は一般メディア判断として扱いますが、VT の代表例としては広げません。一方でチケット、遠征、会員/サブスク、スパチャ、ボイス系デジタル商品は対象外です。",
    shortScopeDisclosure: "VT は主に CD / BD を media 枠で対応。チケット・遠征・会員/スパチャ・ボイスは対象外です。",
