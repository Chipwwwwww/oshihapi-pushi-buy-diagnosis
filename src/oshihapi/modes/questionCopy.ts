import type { PresentationMode as ModeId } from "@/src/oshihapi/modes/presentationMode";

type OptionCopy = {
  label?: string;
};

type QuestionCopy = {
  title?: string;
  helper?: string;
  options?: Record<string, OptionCopy>;
};

export const QUESTION_COPY: Record<ModeId, Record<string, QuestionCopy>> = {
  standard: {},
  kawaii: {
    q_desire: {
      title: "これ、どれくらいトキメク？",
      helper: "0=ちょっと気になる / 5=最推し級だよ",
    },
    q_budget_pain: {
      title: "このお買い物、あとでしんどくならない？",
      options: {
        ok: { label: "ぜんぜん平気" },
        some: { label: "ちょっとだけ痛い" },
        hard: { label: "生活にひびきそう" },
        force: { label: "ムリして払うかも" },
      },
    },
    q_urgency: {
      title: "今買わないとどうなるかな？",
      options: {
        not_urgent: { label: "あとでも買えそう" },
        low_stock: { label: "在庫へってる/締切近い" },
        last: { label: "ほぼラスト/今日まで" },
        unknown: { label: "まだわからない" },
      },
    },
    q_rarity_restock: { title: "また会えそう？" },
    q_goal: { title: "今回の買い方、どれが近いかな？", options: { single: { label: "推し1点狙い（本命だけ）" }, set: { label: "セットでそろえたい（まとめ買い）" }, fun: { label: "買う過程も楽しみたい（ワクワクしたい）" }, unknown: { label: "まだわからない" } } },
    q_motives_multi: {
      title: "買いたい気持ち、どれに近い？（いくつでもOKだよ）",
      options: {
        bonus: { label: "特典が本体" },
        fomo: { label: "限定・締切がこわい" },
        rush: { label: "買えた瞬間のドキドキ" },
        support: { label: "推しを応援したい" },
        trend: { label: "みんな買ってる" },
        use: { label: "飾りたい／使いたい" },
        vague: { label: "なんとなく" },
        memory: { label: "記念・思い出として残したい" },
        content: { label: "中身（絵/文章/デザイン）が好き" },
        complete: { label: "揃えたい／コンプしたい" },
        display: { label: "飾って眺めたい" },
        archive: { label: "手元に保存・アーカイブしたい" },
        reward: { label: "自分へのご褒美" },
        scarce: { label: "今逃すと後悔しそう" },
        price_up: { label: "あとで高くなりそう" },
        habit: { label: "習慣でつい買ってしまう" },
      },
    },
    q_hot_cold: {
      title: "これ、あとで高くなりそう？（相場/プレ値）",
      options: {
        hot: { label: "高騰しやすい（プレ値になりやすい）" },
        normal: { label: "普通（大きくは動かなそう）" },
        cold: { label: "落ち着いてる（上がりにくい/安定しそう）" },
        unknown: { label: "わからない（相場未確認）" },
      },
    },
    q_impulse_axis_short: {
      title: "今の気持ち、どれに近い？",
      options: {
        plan: { label: "見返す/保管までイメージできてる（計画型）" },
        mood: { label: "今のテンションで欲しい（計画はまだ）" },
        both: { label: "両方ある（計画もあるし、今も高まってる）" },
        unknown: { label: "まだわからない" },
      },
    },
    q_regret_impulse: { title: "いまの気分、どれが近い？" },
    q_long_note: { title: "AIに相談したいことをメモしよう" },

    q_storage_fit: { title: "保管プランある？（ファイル/スキャン/置き場所）", options: { CONFIRMED: { label: "あるよ！（もう決めた）" }, PROBABLE: { label: "たぶんある…！（片付ければ）" }, NONE: { label: "いまはない…" }, UNKNOWN: { label: "わかんない（先に見てくる）" } } },
    q_price_feel: { title: "お値段の納得感はどう？", options: { good: { label: "いい感じ" }, normal: { label: "ふつう" }, high: { label: "ちょっと高め" }, unknown: { label: "まだ比べてない" } } },
    q_storage_space: { title: "置き場所だいじょうぶ？", options: { enough: { label: "問題ない" }, adjust: { label: "少し工夫する" }, tight: { label: "かなりきびしい" } } },
    q_alternative_plan: { title: "見送るなら別プランある？", options: { clear: { label: "あるよ" }, maybe: { label: "たぶんある" }, none: { label: "ないかも" } } },
    q_addon_common_info: { title: "必要な情報、そろってる？" },
    q_addon_common_priority: { title: "今月の優先度は高い？" },
    q_addon_goods_compare: { title: "似てるグッズと比べられた？" },
    q_addon_goods_portability: { title: "持ち帰り対策（折れ・濡れ）できそう？" },

    q_addon_goods_collection_goal: { title: "集める目的、どれが近い？" },
    q_addon_goods_trade_intent: { title: "交換する気持ち、ある？" },
    q_addon_goods_paper_care: { title: "紙もの、どうしまっておく？" },
    q_addon_goods_paper_view_freq: { title: "あとで見返すことある？" },
    q_addon_goods_wear_freq: { title: "どのくらい使いそう？" },
    q_addon_goods_wear_fit: { title: "サイズや雰囲気、合いそう？" },
    q_addon_goods_display_space_plan: { title: "保管と配送/搬入の見通しは？" },
    q_addon_goods_display_move_risk: { title: "配送・搬送と掃除負担は？" },
    q_addon_goods_tech_compat: { title: "対応チェックできてる？" },
    q_addon_goods_tech_after_use: { title: "現場のあとも使う？" },
    q_addon_goods_itabag_target: { title: "完成イメージはどのくらい決まってる？" },
    q_addon_goods_itabag_usage: { title: "現場で使う？展示メイン？" },
    q_addon_goods_wear_scene: { title: "使う場面はライブ？普段？" },
    q_addon_blind_draw_trade_intent: { title: "余りを交換する予定ある？" },
    q_addon_blind_draw_duplicate_tolerance: { title: "被り、どこまで平気？" },
    q_addon_blind_draw_exchange_friction: { title: "交換の手間、どこまで平気？" },
    q_addon_blind_draw_single_fallback: { title: "あとで単品回収に切り替えられる？" },
    q_addon_blind_draw_stop_budget: { title: "止める予算ライン、ある？" },
    q_addon_media_single_vs_set_intent: { title: "今回は1点・ベスト1種・選抜回収・フルセットのどれに近い？" },
    q_addon_media_completion_satisfaction: { title: "全部そろわなくても満足して止まれそう？" },
    q_addon_media_used_market_recovery: { title: "逃した分をあとで埋める前提、ある？" },
    q_addon_media_used_market_comfort: { title: "あとで中古で探すの、どこまで平気？" },
    q_addon_media_completion_pressure_type: { title: "揃えたい圧の正体、どれが近い？" },
    q_addon_media_set_reward_strength: { title: "フルセット報酬って、どれくらい実体ある？" },
    q_addon_voice_cd_kind: { title: "今回ほしいのは、どのドラマCD文脈に近い？" },
    q_addon_voice_cast_check: { title: "出演キャスト確認、どれくらい大事？" },
    q_addon_voice_bonus_is_audio: { title: "音声特典や店舗特典ドラマ、どれくらい大事？" },
    q_addon_voice_listen_timing: { title: "このドラマCD、いつ頃聴くイメージ？" },
    q_addon_used_defect_return: { title: "キズ許容と返品条件、確認できてる？" },
    q_addon_used_authenticity: { title: "正規品の証明ってどのくらい重要？" },
    q_addon_blind_draw_cap: { title: "盲抽にどれくらい慣れてる？" },
    q_addon_blind_draw_exit: { title: "やめどきライン、ある？" },
    q_addon_ticket_schedule: { title: "日程や移動、だいじょうぶ？" },
    q_addon_ticket_resale_rule: { title: "キャンセル・譲渡ルール見た？" },
    q_addon_preorder_timeline: { title: "到着まで待てそう？" },
    q_addon_preorder_restock: { title: "再販の可能性しらべた？" },
    q_addon_used_condition: { title: "中古の状態、受け入れられる？" },
    q_addon_used_price_gap: { title: "新品との差額、納得できる？" },
    gb_q1_need: { title: "今回の課金目的、はっきりしてる？" },
    gb_q2_type: { title: "どのタイプに課金する？", options: { gacha: { label: "ガチャ" }, pass: { label: "月パス系" }, skin: { label: "スキン" }, pack: { label: "お得パック" }, other: { label: "その他" } } },
    gb_q3_budget: { title: "お財布への負担はどう？", options: { easy: { label: "らくらく払える" }, ok: { label: "調整すればOK" }, hard: { label: "ちょっと重い" } } },
    gb_q4_use: { title: "使う見込みはどれくらい？" },
    gb_q5_now: { title: "いまのテンションは？", options: { calm: { label: "落ち着いてる" }, up: { label: "ちょい上がり" }, rush: { label: "今すぐ決めたい" } } },
    gb_q6_repeat: { title: "過去の課金、満足できた？" },
    gb_q7_alt: { title: "他の手段はある？" },
    gb_q8_wait: { title: "少し待つと気持ち変わりそう？" },
    gb_q9_info: { title: "必要な情報、確認できた？" },
    gb_q10_pity: { title: "天井までの距離は？" },
    gb_q10_value: { title: "内容の納得感は？" },
  },
  oshi: {
    q_desire: {
      title: "この案件、推し熱どのくらい？",
      helper: "0=様子見 / 5=最優先で回収したい",
    },
    q_budget_pain: {
      title: "この出費、オタ活資金にダメージある？",
      options: {
        ok: { label: "ノーダメ" },
        some: { label: "ちょいダメージ" },
        hard: { label: "今月しんどい" },
        force: { label: "無理課金コース" },
      },
    },
    q_urgency: {
      title: "今回収しないと取り逃しそう？",
      options: {
        not_urgent: { label: "まだ追える" },
        low_stock: { label: "残数少なめ/締切近い" },
        last: { label: "実質ラストチャンス" },
        unknown: { label: "情報不足" },
      },
    },
    q_rarity_restock: { title: "再販導線、ありそう？" },
    q_goal: { title: "今回の回収スタイル、どれ？", options: { single: { label: "本命1点狙い" }, set: { label: "セット/複数回収" }, fun: { label: "過程/体験重視" }, unknown: { label: "該当なし/保留" } } },
    q_motives_multi: {
      title: "回収動機どれ？（複数OK／迷ったら少なめでもOK）",
      options: {
        bonus: { label: "特典本体" },
        fomo: { label: "取り逃し圧が強い" },
        rush: { label: "取得時の高揚" },
        support: { label: "推し支援" },
        trend: { label: "界隈トレンド" },
        use: { label: "運用・展示したい" },
        vague: { label: "言語化しづらい" },
        memory: { label: "記念保存" },
        content: { label: "内容物が刺さる" },
        complete: { label: "収集完成欲" },
        display: { label: "展示鑑賞目的" },
        archive: { label: "アーカイブ保全" },
        reward: { label: "自己報酬" },
        scarce: { label: "再入手難の懸念" },
        price_up: { label: "将来相場上昇見込み" },
        habit: { label: "習慣購買" },
      },
    },
    q_hot_cold: {
      title: "相場（プレ値）リスク、どの温度感？",
      options: {
        hot: { label: "高騰しやすい（プレ値になりやすい）" },
        normal: { label: "普通（大きくは動かなそう）" },
        cold: { label: "落ち着いてる（上がりにくい/安定しそう）" },
        unknown: { label: "わからない（相場未確認）" },
      },
    },
    q_impulse_axis_short: {
      title: "今の気持ち、どれに近い？",
      options: {
        plan: { label: "見返す/保管までイメージできてる（計画型）" },
        mood: { label: "今のテンションで欲しい（計画はまだ）" },
        both: { label: "両方ある（計画もあるし、今も高まってる）" },
        unknown: { label: "まだわからない" },
      },
    },
    q_regret_impulse: { title: "現在のメンタル状態、どれに近い？" },
    q_long_note: { title: "AI相談用メモを残す" },

    q_storage_fit: { title: "保管プランある？（ファイル/スキャン/置き場所）", options: { CONFIRMED: { label: "確保済み（勝ち）" }, PROBABLE: { label: "片付ければいける" }, NONE: { label: "今はムリ（圧迫）" }, UNKNOWN: { label: "未確認（先に確認）" } } },
    q_price_feel: { title: "価格感、相場的にどう？", options: { good: { label: "納得" }, normal: { label: "標準" }, high: { label: "やや高" }, unknown: { label: "未比較" } } },
    q_storage_space: { title: "保管キャパは確保できる？", options: { enough: { label: "問題なし" }, adjust: { label: "調整が必要" }, tight: { label: "厳しい" } } },
    q_alternative_plan: { title: "見送り時の代替案は？", options: { clear: { label: "明確にある" }, maybe: { label: "候補あり" }, none: { label: "特にない" } } },
    q_addon_common_info: { title: "判断材料は十分に揃ってる？" },
    q_addon_common_priority: { title: "今月案件の中で優先度は？" },
    q_addon_goods_compare: { title: "類似案件との比較は済んだ？" },
    q_addon_goods_portability: { title: "持ち帰り対策（折れ・濡れ）の見通しは？" },

    q_addon_goods_collection_goal: { title: "回収目的はどれに近い？" },
    q_addon_goods_trade_intent: { title: "交換・トレード意欲は？" },
    q_addon_goods_paper_care: { title: "紙もの保管プランある？" },
    q_addon_goods_paper_view_freq: { title: "見返し頻度は？" },
    q_addon_goods_wear_freq: { title: "運用頻度の見込みは？" },
    q_addon_goods_wear_fit: { title: "サイズ・雰囲気の適合度は？" },
    q_addon_goods_display_space_plan: { title: "保管と配送/搬入の見通しは？" },
    q_addon_goods_display_move_risk: { title: "配送・搬送と掃除負担は？" },
    q_addon_goods_tech_compat: { title: "互換確認の進捗は？" },
    q_addon_goods_tech_after_use: { title: "現場後の運用予定は？" },
    q_addon_goods_itabag_target: { title: "完成イメージの具体度は？" },
    q_addon_goods_itabag_usage: { title: "現場運用？展示運用？" },
    q_addon_goods_wear_scene: { title: "使用文脈はライブ/日常のどちら？" },
    q_addon_blind_draw_trade_intent: { title: "余剰分の交換意向は？" },
    q_addon_blind_draw_duplicate_tolerance: { title: "重複許容度は？" },
    q_addon_blind_draw_exchange_friction: { title: "交換実務の負担許容は？" },
    q_addon_blind_draw_single_fallback: { title: "停止後の単品/中古移行意向は？" },
    q_addon_blind_draw_stop_budget: { title: "停止予算ラインは設定済み？" },
    q_addon_media_single_vs_set_intent: { title: "1点回収 / ベスト1種 / 選抜回収 / フルセットのどれ寄り？" },
    q_addon_media_completion_satisfaction: { title: "非フルコンプでも満足停止できる？" },
    q_addon_media_used_market_recovery: { title: "欠けた分を後追い回収する前提は？" },
    q_addon_media_used_market_comfort: { title: "二次流通での後追い回収許容は？" },
    q_addon_media_completion_pressure_type: { title: "completion pressure の主因は？" },
    q_addon_media_set_reward_strength: { title: "セット報酬の実体強度は？" },
    q_addon_voice_cd_kind: { title: "対象はどの drama-CD 文脈？" },
    q_addon_voice_cast_check: { title: "出演キャスト確認の重要度は？" },
    q_addon_voice_bonus_is_audio: { title: "音声 bonus / 店舗特典ドラマの重要度は？" },
    q_addon_voice_listen_timing: { title: "実際の listen timing は？" },
    q_addon_used_defect_return: { title: "瑕疵許容と返品規約の確認状況は？" },
    q_addon_used_authenticity: { title: "真贋証明の重要度は？" },
    q_addon_blind_draw_cap: { title: "盲抽への慣れは？" },
    q_addon_blind_draw_exit: { title: "コンプ目的か本命狙いか？" },
    q_addon_ticket_schedule: { title: "日程・移動の実行性は？" },
    q_addon_ticket_resale_rule: { title: "キャンセル・譲渡規約は確認済み？" },
    q_addon_preorder_timeline: { title: "発送予定・再販予定は確認済み？" },
    q_addon_preorder_restock: { title: "再販導線の確認状況は？" },
    q_addon_used_condition: { title: "中古状態リスクは許容範囲？" },
    q_addon_used_price_gap: { title: "新品差額の妥当性は？" },
    gb_q1_need: { title: "今回の課金目的、明確？" },
    gb_q2_type: { title: "どの施策を回す？", options: { gacha: { label: "ガチャ案件" }, pass: { label: "月パス系" }, skin: { label: "スキン系" }, pack: { label: "パック案件" }, other: { label: "その他" } } },
    gb_q3_budget: { title: "資金圧はどの程度？", options: { easy: { label: "余裕あり" }, ok: { label: "調整で対応" }, hard: { label: "圧が高い" } } },
    gb_q4_use: { title: "活用見込みはある？" },
    gb_q5_now: { title: "いまの熱量は？", options: { calm: { label: "平常" }, up: { label: "高まり中" }, rush: { label: "即断モード" } } },
    gb_q6_repeat: { title: "過去課金の満足度は？" },
    gb_q7_alt: { title: "代替手段はある？" },
    gb_q8_wait: { title: "待機すると温度は変わる？" },
    gb_q9_info: { title: "必要情報の確認状況は？" },
    gb_q10_pity: { title: "天井距離はどの位置？" },
    gb_q10_value: { title: "施策内容の納得感は？" },
  },
};
