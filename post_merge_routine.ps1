...
if ($Expect.Count -gt 0) {
  ...
  foreach ($pat in $Expect) {
    ...
    if ($paths.Count -gt 0) { & git grep -n -- "$pat" -- @paths | Out-Null }
    else { & git grep -n -- "$pat" | Out-Null }
...
if (-not $SkipPull) {
  if ($gitOk) {
    ...
    if ($LASTEXITCODE -eq 0 -and $up) { Run "git" @("pull","--ff-only") }
    else { Write-Host "No upstream branch; skip git pull." -ForegroundColor DarkGray }
  } else {
